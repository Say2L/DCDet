...
class DatasetTemplate(torch_data.Dataset):
    def __init__(self, dataset_cfg=None, class_names=None, training=True, root_path=None, logger=None):
        ...
        self.valid_class_name = copy.deepcopy(self.class_names)
        if self.dataset_cfg.DATA_AUGMENTOR.get('USE_SIMILAR_AUG', False) and 'Car' in self.class_names:
            self.valid_class_name.append('Van')
    ...
    def prepare_data(self, data_dict):
        ...
        if self.training:
            ...
            gt_boxes_mask = np.array([n in self.valid_class_name for n in data_dict['gt_names']], dtype=np.bool_)
        ...
        if data_dict.get('gt_boxes', None) is not None:
            if not self.training:
                self.valid_class_name = self.class_names
            selected = common_utils.keep_arrays_by_name(data_dict['gt_names'], self.valid_class_name)
            data_dict['gt_boxes'] = data_dict['gt_boxes'][selected]
            data_dict['gt_names'] = data_dict['gt_names'][selected]
            gt_classes = np.array([self.valid_class_name.index(n) + 1 for n in data_dict['gt_names']], dtype=np.int32)

            if self.training and self.dataset_cfg.DATA_AUGMENTOR.get('USE_SIMILAR_AUG', False) and 'Car' in self.valid_class_name:
                similar_boxes_mask = np.array([n in ['Car', 'Van'] for n in data_dict['gt_names']], dtype=np.bool_)
                gt_classes[similar_boxes_mask] = 1
        ...
        data_dict.pop('difficulty', None)
        return data_dict
    
    def prepare_data_backup(self, data_dict):
        """
        Args:
            data_dict:
                points: optional, (N, 3 + C_in)
