...
class PillarRes18BackBone8xV1(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
        super().__init__()
        self.model_cfg = model_cfg
        norm_fn = partial(nn.BatchNorm1d, eps=1e-3, momentum=0.01)
...
class PillarNeXtBackbone8x(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
        super().__init__()
        self.model_cfg = model_cfg
        norm_fn = partial(nn.BatchNorm1d, eps=1e-3, momentum=0.01)
        ...
        self.conv1 = spconv.SparseSequential(
            block(input_channels, 64, 3, norm_fn=norm_fn, stride=1, padding=1, indice_key='spconv1', conv_type='spconv'),
        ...
        self.mapping = block(256, 256, 1, norm_fn=norm_fn, stride=1, padding=0, indice_key='mapping', conv_type='spconv')
