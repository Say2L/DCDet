...
class SeparateHead(nn.Module):
    def __init__(self, input_channels, sep_head_dict, stride=1, init_bias=-2.19, use_bias=False, norm_func=None):
        super().__init__()
...
class CenterHead(nn.Module):
    def __init__(self, model_cfg, input_channels, num_class, class_names, grid_size, point_cloud_range, voxel_size,
        ...
        self.blur_parameter = [torch.from_numpy(np.array(blur)) for blur in self.model_cfg.get('BLUR_PARAMETER', [[0.5, 0.5, 0.5]])]
    ...
    def assign_targets(self, gt_boxes, feature_map_size=None, **kwargs):
        ...
        for head_idx, cur_class_names in enumerate(self.class_names_each_head):
            ...
            for bs_idx in range(batch_size):
                ...
                head_feature_map_size = [cur_size * self.up_strides[head_idx] for cur_size in feature_map_size]
                heatmap, ret_boxes, inds, mask, ret_boxes_src = self.assign_target_of_single_head(
                    num_classes=len(cur_class_names), gt_boxes=gt_boxes_single_head.cpu(),
                    feature_map_size=head_feature_map_size, feature_map_stride=target_assigner_cfg.FEATURE_MAP_STRIDE,
    ...
    def get_loss(self):
        ...
        for idx, pred_dict in enumerate(pred_dicts):
            ...
            pred_boxes = pred_boxes.view(pred_boxes.size(0), -1, pred_boxes.size(-1))
            inds = target_dicts['inds'][idx].unsqueeze(2)
            reg_inds = inds.expand(inds.size(0), inds.size(1), pred_boxes.size(-1))
            pred_boxes = pred_boxes.gather(1, reg_inds)
            pos_masks = target_dicts['masks'][idx]

            target_cls =  target_dicts['heatmaps'][idx].permute(0, 2, 3, 1).contiguous()
            target_cls = target_cls.view(target_cls.size(0), -1, target_cls.size(-1))
            cls_inds = inds.expand(inds.size(0), inds.size(1), target_cls.size(-1))
            target_cls = target_cls.gather(1, cls_inds)
            target_labels = target_cls.argmax(-1)
