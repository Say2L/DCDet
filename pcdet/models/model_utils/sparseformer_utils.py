...
@torch.no_grad()
def get_slot_coors(coors, sparse_shape, slot_shape, slot_direction):  
    ...
    if slot_direction == 0:
        slot_coors = coors[:, 3]
        sparse_shape_slot = sparse_shape_x
        coorsx_in_slot = slot_coors % slot_shape
        coors_in_slot = torch.cat([coorsx_in_slot[:, None], coors[:, [2]], coors[:, [1]]], dim=1)  # N, xyz
    else:
        slot_coors = coors[:, 2]
        sparse_shape_slot = sparse_shape_y
        coorsy_in_slot = slot_coors % slot_shape
        coors_in_slot = torch.cat([coorsy_in_slot[:, None], coors[:, [3]], coors[:, [1]]], dim=1)  # N, yxz

    max_slot_num = int(np.ceil(sparse_shape_slot / slot_shape))
    batch_slot_inds = coors[:, 0] * max_slot_num + slot_coors // slot_shape

    return batch_slot_inds, coors_in_slot
