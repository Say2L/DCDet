...
def DCDetIoUregLoss(pred_boxes, gt_boxes, mask):
    ...
    iou = box_utils.bbox3d_overlaps_diou(pred_boxes[mask.bool()], gt_boxes[mask.bool()])
...
class DynamicPositiveMask(nn.Module):
    ...
    def rwiou_cost(self, pred_reg, gt_reg, mask, r_factor):
    ...
    def forward(self, pred_cls, target_cls, pred_reg, gt_reg, masks, iou_target, r_factor=1):
        with torch.no_grad():
...
class RWIoULoss(nn.Module):
    ...
    def forward(self, pred_reg, gt_reg, mask, r_factor=1):
        isnotnan = (~ torch.isnan(gt_reg)).float().all(dim=-1)
